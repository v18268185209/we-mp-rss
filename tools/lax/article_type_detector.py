...
class ArticleTypeDetector:
    ...
    def _detect_video(self, html, soup, features):
        ...
        # 检查HTML中的视频特征字符串
        video_strings = ['video_iframe', 'mpvideo', 'data-type="video"', 'mpvid']
        for vs in video_strings:
            if vs in html:
                features['video_count'] += 1
                break
    ...
    def _detect_audio(self, html, soup, features):
        ...
        # 检查HTML中的音频特征字符串
        audio_strings = ['mpvoice', 'data-type="audio"', 'voice_encode_fileid']
        for as_ in audio_strings:
            if as_ in html:
    ...
    def _detect_images(self, html, soup, features):
        ...
        # 检查HTML中的图片特征字符串
        image_strings = ['rich_pages', 'data-type="jpeg"', 'data-type="png"']
        for is_ in image_strings:
            if is_ in html:
                features['image_count'] += 1
                break
    ...
    def _detect_share(self, html, soup):
        ...
        # 检查分享链接特征
        if 'data-type="share"' in html:
            return True
        
        # 检查是否是分享卡片
        share_card = soup.find('div', class_='share_card')
        if share_card:
            return True
...
def main():
    ...
    for i, url in enumerate(urls, 1):
        ...
        if result['success']:
            ...
            print(f"文章{i}: 分析失败 - {result['error']}")
